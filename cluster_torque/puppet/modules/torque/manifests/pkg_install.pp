...
class torque::pkg_install {
	...
	file { "/tmp/torque-${version}_${arch}.deb":
		...
		source => "puppet:///modules/torque/torque-${version}_${arch}.deb",
	}

	file { "/tmp/torque_dev-${version}_${arch}.deb":
		ensure => present,
		source => "puppet:///modules/torque/torque_dev-${version}_${arch}.deb",
	}

	file { "/tmp/torque_doc-${version}_${arch}.deb":
		ensure => present,
		source => "puppet:///modules/torque/torque_doc-${version}_${arch}.deb",
	}

	file { "/tmp/torque_initd-${version}_${arch}.deb":
		ensure => present,
		source => "puppet:///modules/torque/torque_initd-${version}_${arch}.deb",
	}
